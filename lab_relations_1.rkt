...
#|-------------------------------------------------------------------------------
 ...
 | Pledge:I pledge my honor that I abide by Stevens honor code.
 |-------------------------------------------------------------------------------|#
...
(define (id n)
  ;(eval n)
  (if (= n 0) (list) (cons (list (eval n) (eval n) ) (id (- n 1)) )  )   )
"good"
...
(define (reflexive? n R)
  ;(car (intersection (id n) R))
  (if (null? (intersection (id n) R)) #F (if (element? (car (intersection (id n) R)) n ) #T (reflexive? n (cdr (intersection (id n) R)) ) ) )
)
...
(define (converse R)
  ;(list (caar R) (cadar R) ) )
  (if (null? R) (list) (cons (list (cadar R) (caar R)) (converse (cdr R)) ) ) )
...
;; 2 PTS
(define (symmetric? R)
  (if (null? R) #t (set-equal?  (converse R) R) ) )
...
(define (symmetric-closure R)
  (if (null? R) (list) (union R (converse R)))
  )
"good"
...
;; 3 PTS
(define (relate x R)
  (if (null? R) (list) (if (equal? (caar R) x) (append (relate x (cdr R)) (list (cadar R))) (cons (relate x (cdr R)) (list)) )  )   )
"good"
...
;; 3 PTS
(define (injective? R)
  (if (null? R) #t (= (length (seconds R)) (cardinality (seconds R))) ) )
  ;(length(make-set(seconds R))))

(define (seconds L)
  (if (null? L) (list) (append (seconds (cdr L)) (list (cadar L)) )))
