...
#|-------------------------------------------------------------------------------
 ...
 | Pledge: I pledge my honor that I have abided by the honor code.
 |-------------------------------------------------------------------------------|#
...
;; 1 PTS
(define (divides? a b)
  (if (and (= a 0) (= b 0) ) #t (if (= a 0) #f (if (= (modulo b a) 0) #t #f)))
  )
...
(define (wj-solvable? cA cB g)
  (if (not(>= (max cA cB) g )) #f
      (if (= (modulo g (gcd cA cB)) 0) #t #f)))
...
;; 4 PTS
(define (waterjug cA cB g) (wj-helper cA cB 0 0 g));(wj-helper cA cB 0 0 g '() ))
"
(define (wj-helper cA cB fA fB g )
  
  (cond
    [ (= fB g) (list fA fB)]

    [ (= fB cB) (cons (list fA fB) (list (wj-helper cA cB fA 0 g)))];fB = 0

    [ (= fA 0) (cons (list fA fB) (list (wj-helper cA cB cA fB g)))];fA = cA

    [ (> (+ fA fB) cB) (cons (list fA fB) (list (wj-helper cA cB (- fA (- cB fB)) cB g)))];fA = fA - (cB - fB) || fB = cB

    [ (<= (+ fA fB) cB) (cons (list fA fB) (list (wj-helper cA cB 0 (+ fB fA) g)))];fA = 0 || fB = fB =fA
    )
  )
"
"
(define (wj-helper cA cB fA fB g moves) ;(append moves (list (list fA fB)) )
  
  (cond
    [ (= fB g) (append moves (list (list fA fB)))]

    [ (= fB cB) (wj-helper cA cB fA 0 g (append moves (list (list fA fB))))];fB = 0

    [ (= fA 0) (wj-helper cA cB cA fB g (append moves (list (list fA fB))))];fA = cA

    [ (> (+ fA fB) cB) (wj-helper cA cB (- fA (- cB fB)) cB g (append moves (list (list fA fB))))];fA = fA - (cB - fB) || fB = cB

    [ (<= (+ fA fB) cB) (wj-helper cA cB 0 (+ fB fA) g (append moves (list (list fA fB))))];fA = 0 || fB = fB =fA
    )
  )
"
(define (wj-helper cA cB fA fB g) ;(append moves (list (list fA fB)) )
  
  (cond
    [ (= fB g) (list (list fA fB))]

    [ (= fB cB) (cons (list fA fB) (wj-helper cA cB fA 0 g))];fB = 0

    [ (= fA 0) (cons (list fA fB) (wj-helper cA cB cA fB g))];fA = cA

    [ (> (+ fA fB) cB) (cons (list fA fB) (wj-helper cA cB (- fA (- cB fB)) cB g))];fA = fA - (cB - fB) || fB = cB

    [ (<= (+ fA fB) cB) (cons (list fA fB) (wj-helper cA cB 0 (+ fB fA) g))];fA = 0 || fB = fB =fA
    )
  )
...
(define (smallest n i)
    (if (= (modulo n i) 0) i (smallest n (+ 1 i)))
  )

(define (factor n)
    (if (= 1 n) (list)
        (append (list (smallest n 2)) (factor (/ n (smallest n 2)) )    ))
  )
