...
(define (mod-exp b e m)
   (cond [(= e 0) (modulo 1 m)]
         ...
         [(and (> e 0) (= 1 (modulo e 2)) ) (modulo (* (mod-exp b (- e 1) m) b) m)])
  )

"
 (cond [(= e 0) (modulo 1 m)]
        [(and (> e 0) (= 0 (modulo e 2)) ) (modulo (expt (modulo (expt b (/ e 2)) m) 2) m)]
        [else (modulo (* (modulo (expt b (- e 1)) m) b) m)])
"
"
 (cond [(= e 0) (modulo 1 m)]
        [(and (> e 0) (= 0 (modulo e 2)) ) (modulo (expt (mod-exp b (/ e 2) m) 2) m)]
...
(define (encrypt plaintext key)
  ;(let ([values (map char->integer (string->list plaintext))]));works
"
    (let (func (lambda (values)
        (cons (mod-exp (car values) (car kay) (cdar key)) (rec (cdr values)))
              ))
  )
"
  (func (map char->integer (string->list plaintext)) key)
  )

(define (func values keys)
; (car values)
 (if (equal? values '() ) '()
  (append  (list (mod-exp (car values)  (cadr keys) (car keys))) (func (cdr values) keys) ))
...
(define (decrypt ciphertext key)
  ...
  (list->string (map integer->char (func1 ciphertext key)))
  )

(define (func1 values keys)
; (car values)
 (if (equal? values '() ) '()
 (append  (list (mod-exp (car values)  (cadr keys) (car keys))) (func (cdr values) keys) ))
