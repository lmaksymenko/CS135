...
;; 5 PTS
(define (compose S R)
  ;for each value in the first list we use relate on the second list
  ;for each value in relate list we return the sets with the number in the first spot

  ;v2
  ;take the second number from each relation in the second list (loop and get pair the relation of the second number of each pair)
  ;get the relation of each of those numbers to the first list (loop through the relation to ^ number and turn in into a set
  ;return the combo of that number with each number in relate
  ...
  (if (or (null? S) (null? R)) '() ;check for second case of null one ahead    relate(x R)
      (if (null? (cdr R)) (helper0 (caar R) (relate (cadar R) S))
          (make-set (append (helper0 (caar R) (relate (cadar R) S)) (compose S (cdr R))))  )
      )
             
  )

(define (helper0 v R);first num of pair, relation
     (if (null? R) '() (cons (list v (car R)) (helper0 v (cdr R)) )  )   ) 
...
(define (transitive-closure R)
  ...
  (if (null? R) '()
      (helper1 (cardinality R) R) ) )

;check null
  ;check cardinality
  ;run power for cardinality (loop)
  ;run union cardinality this func one less
...
(define (equivalence? n R)
  ...
  (and (reflexive? n R) (and (symmetric? R) (transitive? R))) )
...
(define (involutive? n R)
  ...
  (set-equal? (id n) (compose R R)))



  ;(if (null? R) #f
     ; (if (not (null? (relate n R))) (and (symmetric? R) (injective? R)) #f)))

;converse-
;injective?
;symmetric?
;create surjective using relate -> for each y in (x y) in R there is a relation
