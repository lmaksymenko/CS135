...
;to the TA that said we cant use and and or statments. Have this instead.
...
(define (p- a b)
  (if (myOWNand (pz? a) (myOWNnot (pz? b))) (error-subtraction) (if (pz? b) a (p- (pred a) (pred b)) ) )
      )
"good"


(define (p-helper a b)
  (if (myOWNand (pz? a) (myOWNnot (pz? b))) #f (if (pz? b) a (p-helper (pred a) (pred b)) ) )
      )
...
;; 2 PTS
(define (p= a b)
  (if (pz? (p-helper a b)) #t #f))

;catch exceptions?
;subtraction the only way to show that 2 numbers are equal, we can only compare 0 at this point and we cannot transition into normal numbers
...
;; 2 PTS
(define (p> a b)
  (if (myOWNand (p-helper a b) (myOWNnot (p= a b)) ) #t #f))
"good"
...
(define (p% a b)
  (if (pz? a) z
      (if (pz? b) (error-modulo)
          (if (myOWNor (p> a b)(p= a b)) (p% (p- a b) b) a)
         )
      )
  )
...
;; 3 PTS
(define (p-prime? p)
  (if (myOWNor (pz? p) (p= (succ z) p)) #f (if (p= (succ (succ z)) p) #t (helper (succ (succ z)) p))  ) );2 special case add

    (define (helper a b)
        (if (p-divides? a b) #f
                 (if (p> (pred b) a) (helper (succ a) b) (myOWNnot (p-divides? a b))  )))
;(helper (n->p 2) (n->p 5))