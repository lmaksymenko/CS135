...
"good"
     (if (negative? x)
          ...
          (if (positive? x) (car'("positive")) (car'("zero"))  ) )
  )
...
(define (gen-fib a b n)
  ...
  (if (= n 0)
      ...
      (let ([av a]
            ...
            [sv 0]
            [nv n])
  "too long but safe (if it aint broke dont fix it)"
        (set! sv (+ av bv) )
        (set! av bv )
        (set! bv sv )
        (set! nv (- n 1) ) 
...
(define (filter P L)
  "good"
(if (null? (cdr L)) (if (P (car L)) (list (car L)) '() )
  (if (P (car L)) (append '() (list (car L)) (filter P (cdr L)) ) (filter P (cdr L)) )
  )
  )